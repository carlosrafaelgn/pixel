...
a+" "+a;this.scrollContainer.resize(toolbarTotalHeight,baseHeight-(toolbarTotalHeight<<1));this.pointerCursor.style.borderWidth=borderWidthCss;this.pointerCursor.style.width=c;this.pointerCursor.style.height=c;a=this.level.objects;for(b=a.length-1;0<=b;b--){c=a[b];const d=this.objectImages[b];d.style.left=css(c.x-iconRadius);d.style.top=css(c.y-iconRadius);UISpriteSheet.resize(d)}super.resize()}attach(){return __awaiter(this,void 0,void 0,function*(){View.glCanvas.style.display="none";if(!this.level){this.level=
yield LevelCache.loadLevelFromOptions(this.loadOptions);this.loadOptions&&(this.loadOptions.levelNameOverride&&(this.level.name=this.loadOptions.levelNameOverride),this.loadOptions=null);var a=this.level.objects;const b=a.length;for(let c=0;c<b;c++)this.addObject(a[c],!0)}this.pointerHandler=new PointerHandler(this.canvas,this.mouseDown.bind(this),this.mouseMove.bind(this),this.mouseUp.bind(this));this.scrollContainer.attach();a=yield loadImage(EditorView.brushesImageDataBase64);this.brushCanvas=
document.createElement("canvas");this.brushCanvas.width=a.width;this.brushCanvas.height=a.height;this.brushContext=this.brushCanvas.getContext("2d",{alpha:!0});this.brushContext.drawImage(a,0,0);this.updateBrushColor(colors[this.lastPencilColor]);this.context=this.canvas.getContext("2d",{alpha:!0});this.context.globalCompositeOperation="source-over";this.context.clearRect(0,0,baseWidth,maxHeight);setContextQuality(this.context,!1);this.level.image&&this.context.drawImage(yield loadImage(this.level.image),
0,0);this.context.globalCompositeOperation=this.contextGlobalCompositeOperation;this.boundDocumentKeyUp&&document.addEventListener("keyup",this.boundDocumentKeyUp,!0)})}detach(){return __awaiter(this,void 0,void 0,function*(){View.glCanvas.style.display="";this.pointerHandler&&(this.pointerHandler.destroy(),this.pointerHandler=null);this.scrollContainer.detach();this.brushContext=this.brushCanvas=this.context=null;this.boundDocumentKeyUp&&document.removeEventListener("keyup",this.boundDocumentKeyUp,
!0)})}destroyInternal(a){this.save()}save(){if(this.dirty||!this.level.image)this.level.modifiedAt=(new Date).getTime(),this.level.createdAt||(this.level.createdAt=this.level.modifiedAt),this.level.thumbnailImage="",this.level.image=this.canvas.toDataURL("image/png"),this.level.processedImage="",this.level.polygons=[],LevelCache.saveEditorLevel(this.level),this.dirty=!1}updatePointerCursor(a,b){const c=cssNumber(.5*EditorView.BrushWidths[this.lastBrushWidth]+1);this.pointerCursor.style.left=a-c+"px";
this.pointerCursor.style.top=b-c+this.scrollContainer.topCss+"px"}mouseDown(a){if(View.loading)return!1;const b=this.scrollContainer.containerElement.getBoundingClientRect(),c=a.clientX-b.left;a=a.clientY-b.top;this.dirty=!0;switch(this.tool){case EditorView.ToolPencil:case EditorView.ToolEraser:this.pointerCursorAttached||(this.pointerCursorAttached=!0,this.updatePointerCursor(c,a),this.scrollContainer.element.appendChild(this.pointerCursor));this.draw((this.pointerLastX=model(c)-1)+1,this.pointerLastY=
model(a));break;default:this.handleObjectToolDown(c,a)}return!0}mouseMove(a){var b=this.scrollContainer.containerElement.getBoundingClientRect();const c=a.clientX-b.left;a=a.clientY-b.top;switch(this.tool){case EditorView.ToolPencil:case EditorView.ToolEraser:b=model(c);const d=model(a);if(this.pointerLastX!==b||this.pointerLastY!==d)this.updatePointerCursor(c,a),this.draw(b,d),this.pointerLastX=b,this.pointerLastY=d;break;default:this.handleObjectToolMove(c,a)}}mouseUp(a){this.objectImageDrag=null;
this.pointerCursorAttached&&(this.pointerCursorAttached=!1,this.scrollContainer.element.removeChild(this.pointerCursor));switch(this.tool){case EditorView.ToolPencil:case EditorView.ToolEraser:return}a=this.level.objects;const b=this.objectImages;b.sort((c,d)=>{c=c.object;d=d.object;return c.type-d.type||d.y-c.y});for(let c=b.length-1,d=1;0<=c;c--,d++){const e=b[c];e.style.zIndex=d.toString();a[c]=e.object}}draw(a,b){a-=this.pointerLastX;b-=this.pointerLastY;var c=Math.abs(a),d=Math.abs(b);if(c||
d){const e=EditorView.BrushWidths[this.lastBrushWidth],f=e>>1,g=EditorView.BrushOffsets[this.lastBrushWidth];c=Math.max(c,d);d=this.pointerLastX;let h=this.pointerLastY;a/=c;for(b/=c;0<c;)c--,d+=a,h+=b,c&1||(this.lastPencilColor===colors.length-1&&(this.lastPencilRainbowIndex++,this.lastPencilRainbowIndex>=rainbowColors.length&&(this.lastPencilRainbowIndex=0),this.updateBrushColor(rainbowColors[this.lastPencilRainbowIndex])),this.context.drawImage(this.brushCanvas,g,0,e,e,(d|0)-f,(h|0)-f,e,e))}}handleObjectToolDown(a,
b){var c=model(a),d=model(b);if(this.tool>=EditorView.ToolBall&&this.tool<=EditorView.ToolCucumber){this.objectImageDrag=this.findObjectWithinRadius(-1,c,d);if(!this.objectImageDrag&&(this.objectImageDrag=this.addObject(new LevelObject(this.lastObjectType,c,d),!1),!this.objectImageDrag))return;c=this.scrollContainer.containerElement.getBoundingClientRect();d=this.objectImageDrag.getBoundingClientRect();this.objectImageDragOffsetXCss=d.left-c.left-a;this.objectImageDragOffsetYCss=d.top-c.top-b}else(a=
...
this.preview||(View.main.appendChild(this.backButton),View.main.appendChild(this.restartButton),View.main.appendChild(this.timeDisplay)))))):this.frameRequest&&(cancelAnimationFrame(this.frameRequest),this.frameRequest=0)}else this.frameRequest=0}}GameView.FinishedThisFrame=1;GameView.FinishedVictory=2;GameView.FinishedLoss=4;GameView.wakeLockSentinel=null;
...
borderWidthCss);this.toolbarTop.style.height=toolbarAvailableHeightCss;this.toolbarTop.style.borderBottomWidth=borderWidthCss;for(let k=a.length-1;0<=k;k--){a[k].style.width=thumbnailWidthCss;a[k].style.borderWidth=borderWidthCss;a[k].style.margin=h;a[k].style.padding=m;a[k].style.lineHeight=iconSizeCss;b[k].width=e;b[k].height=f;b[k].style.margin=l;c[k].style.borderTopWidth=borderWidthCss;c[k].style.padding=l;d[k].style.borderWidth=g;d[k].style.padding=buttonPaddingCss;UISpriteSheet.resize(d[k].firstChild);
...
a.name:"-"));b.appendChild(document.createElement("br"));UISpriteSheet.create(UISpriteSheet.Clock,b);b.appendChild(document.createTextNode(a?a.fmt:"-"))}}createThumbnail(a,b,c=null,d=-1){const e=document.createElement("div"),f=document.createElement("div"),g=document.createElement("div"),h=document.createElement("img"),l=document.createElement("div"),m=document.createElement("a"),k=b?d.toString():a;e.className="thumbnail";b?(e.setAttribute("data-id",k),e.setAttribute("data-name",Strings.LevelSpace+
(d+1))):e.setAttribute("data-name",k);prepareButtonBlink(e,!1,this.boundPlay);f.className="thumbnail-preview";g.className="thumbnail-text";g.textContent=a;f.appendChild(g);c?h.src=c:LevelCache.loadLevelInfo(a,!1).then(n=>{n&&(h.src=n.thumbnailImage)},n=>{console.log(n)});f.appendChild(h);UISpriteSheet.create(UISpriteSheet.Menu,m);m.className="menu";prepareButtonBlink(m,!1,this.boundShowMenu);f.appendChild(m);e.appendChild(f);l.className="thumbnail-text thumbnail-record";this.updateThumbnailRecord(k,
l);e.appendChild(l);this.thumbnails.push(e);this.thumbnailImages.push(h);this.thumbnailRecords.push(l);this.anchors.push(m);b||this.hr||(this.hr=document.createElement("hr"),this.scrollContainer.containerElement.appendChild(this.hr));this.scrollContainer.containerElement.appendChild(e)}attach(){return __awaiter(this,void 0,void 0,function*(){if(this.thumbnails)this.lastPlayedThumbnail&&((a=this.lastPlayedThumbnail.getElementsByClassName("thumbnail-record"))&&a[0]&&this.updateThumbnailRecord(this.lastPlayedThumbnail.getAttribute("data-id")||
