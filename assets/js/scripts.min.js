...
this.level&&this.totalElapsedMilliseconds&&this.victory&&this.victory[0]))return null;const a=LevelCache.getLevelRecord(this.level.name);return a&&a.time<=this.totalElapsedMilliseconds[0]?null:LevelCache.LastRecordName||""}setTimeDisplayTextRecord(a){this.timeDisplayText.nodeValue=LevelCache.formatLevelRecordTime(this.totalElapsedMilliseconds[0])+" - "+(a||Strings.NoName)}editName(a){return this.alive&&this.level&&this.totalElapsedMilliseconds&&this.victory&&this.victory[0]?Modal.show({title:Strings.NewRecord,
html:`<label for="name">${Strings.Name}</label><input id="name" spellcheck="false" autocomplete="off" maxlength="8" /><br/><label>${Strings.Time}</label><label>${LevelCache.formatLevelRecordTime(this.totalElapsedMilliseconds[0])}</label>`,okcancel:!0,okcancelsubmit:!0,onshowing:()=>{const b=Modal.currentModalHeaderElement;if(b){b.setAttribute("data-style","line-height:"+iconSize);const c=UISpriteSheet.create(UISpriteSheet.Trophy);c.setAttribute("data-style","margin-right:"+buttonMargin);b.insertBefore(c,
b.firstChild);UISpriteSheet.create(UISpriteSheet.Trophy,b).setAttribute("data-style","margin-left:"+buttonMargin)}document.getElementById("name").value=LevelCache.LastRecordName||""},onshown:()=>{document.getElementById("name").focus()},onok:()=>__awaiter(this,void 0,void 0,function*(){if(this.alive&&this.level&&this.totalElapsedMilliseconds){var b=document.getElementById("name").value.trim();b?LevelCache.setLevelRecord(this.level.name,this.totalElapsedMilliseconds[0],b):(LevelCache.deleteLevelRecord(this.level.name),
...
cLib._render(a.verticesPtr,b.levelPtr,LevelSpriteSheet.LevelSpriteSheetPtr,scaleFactor)&&(this.finished=!0,this.pointerCursorAttached[0]=0,this.preview?this.pause(null):(this.totalElapsedMilliseconds&&(b=this.checkRecord(),null===b?(UISpriteSheet.change(this.timeDisplayImage,UISpriteSheet.Clock),this.timeDisplayText.nodeValue=LevelCache.formatLevelRecordTime(this.totalElapsedMilliseconds[0]),this.editNameButton.style.display="none"):(b&&LevelCache.setLevelRecord(this.level.name,this.totalElapsedMilliseconds[0],
b),UISpriteSheet.change(this.timeDisplayImage,UISpriteSheet.Trophy),this.setTimeDisplayTextRecord(b),this.editNameButton.style.display="")),this.finalUIAttached||(this.finalUIAttached=!0,this.preview||(this.baseElement.appendChild(this.backButton),this.baseElement.appendChild(this.restartButton),this.baseElement.appendChild(this.timeDisplay)))))):this.frameRequest&&(cancelAnimationFrame(this.frameRequest),this.frameRequest=0)}else this.frameRequest=0}}GameView.FinishedThisFrame=1;
