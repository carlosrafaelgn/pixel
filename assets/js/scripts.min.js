...
this.element)){if(this.downCallback&&!this.downCallback(a))return cancelEvent(a);this.captured=!0;document.body.addEventListener(this.documentMoveEvent,this.boundDocumentMove,{capture:!0,passive:!1});document.body.addEventListener(this.documentUpEvent,this.boundDocumentUp,!0);this.documentCancelEvent&&document.body.addEventListener(this.documentCancelEvent,this.boundDocumentUp,!0);return cancelEvent(a)}}mouseMove(a){if(this.captured)return this.moveCallback&&this.moveCallback(a),cancelEvent(a)}mouseUp(a){if(this.captured)return document.body.removeEventListener(this.documentUpEvent,
...
class GameView extends View{constructor(a,b){super();this.alive=!0;this.alreadyCreated=this.finished=this.paused=!1;this.viewY=this.level=this.loadOptions=this.editNameButton=this.timeDisplayText=this.timeDisplayImage=this.timeDisplay=this.restartButton=this.backButton=null;this.frameRequest=0;this.finalUIAttached=!1;this.levelTexture=this.resourceStorage=this.boundRender=this.pointerCursorY=this.pointerCursorX=this.pointerCursorCenterY=this.pointerCursorCenterX=this.victory=this.totalElapsedMilliseconds=
this.pointerCursorAttached=this.pointerHandler=null;this.baseElement.style.cursor="crosshair";this.baseElement.style.touchAction="none";var c=this.createButton(b?this.baseElement:null,UISpriteSheet.Back,this.back.bind(this));c.style.position="absolute";c.style.left="0";c.style.top="0";this.backButton=c;this.preview||(c=this.createButton(null,UISpriteSheet.Restart,this.restart.bind(this)),c.style.position="absolute",c.style.top="0",this.restartButton=c,c=document.createElement("div"),c.style.position=
"absolute",this.timeDisplayImage=UISpriteSheet.create(UISpriteSheet.Clock,c),c.appendChild(this.timeDisplayText=document.createTextNode(Strings.Time)),this.editNameButton=this.createButton(c,UISpriteSheet.Edit,this.editName.bind(this)),this.timeDisplay=c);c=this.createButton(this.baseElement,UISpriteSheet.Pause,this.pause.bind(this));c.style.position="absolute";c.style.right="0";c.style.top="0";this.resourceStorage=new ResourceStorage;this.boundRender=this.render.bind(this);this.loadOptions=a;this.preview=
b}get usesGL(){return!0}releaseResources(){this.alive&&this.resourceStorage.release()}loadResources(){this.alive&&(this.resourceStorage.load(),LevelSpriteSheet.LevelModelCoordinates.setCoordinates(0,0,this.levelTexture.width*scaleFactor,this.levelTexture.height*scaleFactor),this.alreadyCreated?(this.frameRequest&&(cancelAnimationFrame(this.frameRequest),this.frameRequest=0),this.frameRequest=requestAnimationFrame(this.boundRender)):(this.alreadyCreated=!0,this.restart()))}resize(){this.restartButton&&
(this.restartButton.style.left=css(buttonHeight+buttonMargin));this.timeDisplay&&(this.timeDisplay.style.marginTop=buttonMarginCss,this.timeDisplay.style.left=css(3*buttonHeight+buttonMargin),this.timeDisplay.style.lineHeight=iconSizeCss,this.editNameButton.style.marginTop="-"+buttonMarginCss,UISpriteSheet.resize(this.timeDisplayImage),this.timeDisplayImage.style.marginRight=buttonMarginCss);this.level.viewResized();super.resize()}attach(){return __awaiter(this,void 0,void 0,function*(){this.level=
yield LevelCache.loadLevelFromOptions(this.loadOptions);this.loadOptions=null;this.pointerHandler=new PointerHandler(this.baseElement,this.mouseDown.bind(this),this.mouseMove.bind(this),this.mouseUp.bind(this));this.levelTexture||(this.levelTexture=new Texture(View.gl,yield loadImage(this.level.processedImage)),this.resourceStorage.add("levelTexture",this.levelTexture));if(androidWrapper)androidWrapper.setKeepScreenOn(!0);else try{const a=navigator.wakeLock;a&&a.request&&!GameView.wakeLockSentinel&&
(GameView.wakeLockSentinel=yield a.request("screen"))}catch(a){}})}detach(){return __awaiter(this,void 0,void 0,function*(){this.pointerHandler&&(this.pointerHandler.destroy(),this.pointerHandler=null);if(androidWrapper)androidWrapper.setKeepScreenOn(!1);else if(GameView.wakeLockSentinel)try{GameView.wakeLockSentinel.release&&(yield GameView.wakeLockSentinel.release(),GameView.wakeLockSentinel=null)}catch(a){}})}destroyInternal(a){this.alreadyCreated=this.alive=!1;this.level.destroyLevelPtr();this.pointerCursorY=
this.pointerCursorX=this.pointerCursorCenterY=this.pointerCursorCenterX=this.victory=this.totalElapsedMilliseconds=this.pointerCursorAttached=this.viewY=null;a?this.resourceStorage.release():this.resourceStorage.destroy()}restart(){if(!this.alive)return!1;this.finalUIAttached&&(this.finalUIAttached=!1,this.preview||this.baseElement.removeChild(this.backButton),this.baseElement.removeChild(this.restartButton),this.baseElement.removeChild(this.timeDisplay));this.finished=this.paused=!1;this.level.restart();
const a=cLib.HEAP8.buffer;let b=cLib._getFirstPropertyPtr(this.level.levelPtr);this.viewY=new Float32Array(a,cLib._getViewYPtr(this.level.levelPtr),1);this.pointerCursorAttached=new Int32Array(a,b,1);b+=4;this.totalElapsedMilliseconds=new Int32Array(a,b,1);b+=4;this.victory=new Int32Array(a,b,1);b+=4;this.pointerCursorCenterX=new Float32Array(a,b,1);b+=4;this.pointerCursorCenterY=new Float32Array(a,b,1);b+=4;this.pointerCursorX=new Float32Array(a,b,1);this.pointerCursorY=new Float32Array(a,b+4,1);
this.frameRequest&&(cancelAnimationFrame(this.frameRequest),this.frameRequest=0);this.frameRequest=requestAnimationFrame(this.boundRender);return!0}back(a){if(!this.alive)return!1;this.alive=!1;this.paused=!0;this.frameRequest&&(cancelAnimationFrame(this.frameRequest),this.frameRequest=0);this.fadeToPrevious();return!0}pause(a){if(!this.alive)return!1;if(!this.paused){const b=ControlMode.accelerationSupported?UISpriteSheet.create(ControlMode.modeImage):null;if(!Modal.show({title:Strings.Pause,html:(androidWrapper||
isPWA?"":`<button type="button" id="fullscreen" data-style="margin-bottom: ${buttonMargin}">${Strings.Fullscreen}</button><br/>`)+(b?`<button type="button" id="controlMode" data-style="margin-bottom: ${buttonMargin}">${Strings.ControlMode}</button><br/>`:"")+`<button type="button" id="restart">${Strings.Restart}</button>`,buttons:[{iconId:UISpriteSheet.Back,text:Strings.Exit,onclick:()=>{Modal.hide();this.back(null)}},{defaultCancel:!0,iconId:UISpriteSheet.PlayGreen,text:Strings.Play,className:"accept",
onclick:()=>{Modal.hide();this.pause(null)}}],onshowing:()=>{let c=document.getElementById("fullscreen");c&&c.insertBefore(UISpriteSheet.create(UISpriteSheet.Fullscreen),c.firstChild);b&&(c=document.getElementById("controlMode"),c.insertBefore(b,c.firstChild));c=document.getElementById("restart");c.insertBefore(UISpriteSheet.create(UISpriteSheet.Restart),c.firstChild)},onbuttonclick:c=>{switch(c){case "fullscreen":fullscreenControl.toggleFullscreen();break;case "controlMode":ControlMode.toggleMode();
UISpriteSheet.change(b,ControlMode.modeImage);break;case "restart":this.restart(),Modal.hide()}}}))return!1}this.paused=!this.paused;this.frameRequest&&(cancelAnimationFrame(this.frameRequest),this.frameRequest=0);this.alive&&(this.frameRequest=requestAnimationFrame(this.boundRender));return!0}mouseDown(a){if(View.loading||!this.alive||this.paused||ControlMode.mode!==ControlMode.Pointer||this.finished)return!1;this.pointerCursorAttached[0]=1;this.pointerCursorCenterX[0]=this.pointerCursorX[0]=model(a.clientX-
baseLeftCss);this.pointerCursorCenterY[0]=this.pointerCursorY[0]=model(a.clientY-baseTopCss);return!0}mouseMove(a){this.pointerCursorX[0]=modelFrac(a.clientX-baseLeftCss);this.pointerCursorY[0]=modelFrac(a.clientY-baseTopCss)}mouseUp(a){this.pointerCursorAttached[0]=0}checkRecord(){if(!(this.alive&&this.level&&this.totalElapsedMilliseconds&&this.victory&&this.victory[0]))return null;const a=LevelCache.getLevelRecord(this.level.name);return a&&a.time<=this.totalElapsedMilliseconds[0]?null:LevelCache.LastRecordName||
