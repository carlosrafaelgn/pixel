...
class PointerHandler{constructor(a,b=null,c=null,d=null){this.documentTarget=document.documentElement||document.body;this.captured=!1;this.pointerId=-1;this.element=a;this.downCallback=b;this.moveCallback=c;this.upCallback=d;"ontouchstart"in a?(this.documentMoveEvent="touchmove",this.documentUpEvent="touchend",this.documentCancelEvent="touchcancel",this.boundDocumentUp=this.touchEnd.bind(this),this.boundDocumentMove=this.touchMove.bind(this),a.ontouchstart=this.touchStart.bind(this)):"onpointerdown"in
a?(this.documentMoveEvent="pointermove",this.documentUpEvent="pointerup",this.documentCancelEvent="pointercancel",this.boundDocumentUp=this.pointerUp.bind(this),this.boundDocumentMove=this.pointerMove.bind(this),a.onpointerdown=this.pointerDown.bind(this)):(this.documentMoveEvent="mousemove",this.documentUpEvent="mouseup",this.documentCancelEvent=null,this.boundDocumentUp=this.mouseUp.bind(this),this.boundDocumentMove=this.mouseMove.bind(this),a.onmousedown=this.mouseDown.bind(this))}destroy(){this.mouseUp(null);
zeroObject(this)}pointerDown(a){if(0<=this.pointerId&&"mouse"!==a.pointerType)return cancelEvent(a);const b=this.mouseDown(a);this.captured&&(this.pointerId=a.pointerId);return b}pointerMove(a){if(this.captured&&a.pointerId===this.pointerId)return this.mouseMove(a)}pointerUp(a){if(this.captured&&a.pointerId===this.pointerId)return this.pointerId=-1,this.mouseUp(a)}touchStart(a){if(!(1<a.touches.length))return 0<=this.pointerId&&this.touchEnd(a),this.pointerId=1,a.clientX=a.touches[0].clientX,a.clientY=
a.touches[0].clientY,a=this.mouseDown(a),void 0===a&&(this.pointerId=-1),a}touchMove(a){if(this.captured&&!(1<a.touches.length))return a.clientX=a.touches[0].clientX,a.clientY=a.touches[0].clientY,this.mouseMove(a)}touchEnd(a){if(this.captured&&!(0>this.pointerId))return this.pointerId=-1,this.mouseUp(a)}mouseDown(a){this.mouseUp(a);if(!(a.button||View.fading||a.target&&a.target!==this.element)){if(this.downCallback&&!this.downCallback(a))return cancelEvent(a);this.captured=!0;this.documentTarget.addEventListener(this.documentMoveEvent,
this.boundDocumentMove,{capture:!0,passive:!1});this.documentTarget.addEventListener(this.documentUpEvent,this.boundDocumentUp,!0);this.documentCancelEvent&&this.documentTarget.addEventListener(this.documentCancelEvent,this.boundDocumentUp,!0);return cancelEvent(a)}}mouseMove(a){if(this.captured)return this.moveCallback&&this.moveCallback(a),cancelEvent(a)}mouseUp(a){if(this.captured)return this.documentTarget.removeEventListener(this.documentUpEvent,this.boundDocumentUp,!0),this.documentTarget.removeEventListener(this.documentMoveEvent,
this.boundDocumentMove,!0),this.documentCancelEvent&&this.documentTarget.removeEventListener(this.documentCancelEvent,this.boundDocumentUp,!0),this.captured=!1,this.upCallback&&this.upCallback(a),cancelEvent(a)}}
...
class EditorView extends View{constructor(a,b){super();this.baseElement.innerHTML='\n\t\t<div class="hidden-container"><input id="fileInput" type="file" accept="image/*" tabindex="-1" /></div>\n\t\t<div id="toolbarTop" class="toolbar toolbar-top"></div>\n\t\t<div id="container"></div>\n\t\t<div id="toolbarBottom" class="toolbar toolbar-bottom"></div>\n\t\t';this.fileInput=this.baseElement.querySelector("#fileInput");this.toolbarTop=this.baseElement.querySelector("#toolbarTop");this.toolbarBottom=
this.baseElement.querySelector("#toolbarBottom");this.scrollContainer=new ScrollContainer(null,!0,"scroll-container-editor",this.baseElement.querySelector("#container"));this.setGridBackground();this.canvas=document.createElement("canvas");this.canvas.width=baseWidth;this.canvas.height=maxHeight;this.scrollContainer.containerElement.appendChild(this.canvas);this.pointerCursor=document.createElement("div");this.pointerCursor.className="pointer-cursor";var c=this.toolbarTop;const d=this.toolbarBottom;
this.createButton(c,UISpriteSheet.Back,this.back.bind(this)).style.float="left";this.firstPencilButtonIndex=this.buttons.length;const e=this.changeToolPencil.bind(this);for(let f=0;f<colors.length-1;f++)this.createButton(c,UISpriteSheet.Border,e,"data-i",f.toString()).firstChild.style.backgroundColor=colorsCss[f];this.createButton(c,UISpriteSheet.Rainbow,e,"data-i",(colors.length-1).toString());this.createButton(c,UISpriteSheet.Eraser,this.changeToolEraser.bind(this));this.firstLineWidthButtonIndex=
this.buttons.length;c=this.changeBrushWidth.bind(this);this.createButton(d,UISpriteSheet.LineWidth0,c,"data-i","0");this.createButton(d,UISpriteSheet.LineWidth1,c,"data-i","1");this.createButton(d,UISpriteSheet.LineWidth2,c,"data-i","2");this.createButton(d,UISpriteSheet.LineWidth3,c,"data-i","3");this.firstObjectToolButtonIndex=this.buttons.length;for(c=LevelObject.TypeBall;c<=LevelObject.LastType;c++)this.createButton(d,LevelObject.ImagesByType[c],this.changeTool.bind(this),"data-tool",(EditorView.ToolBall+
c-LevelObject.TypeBall).toString());this.buttonsWithLargeMargin.push(this.buttons[this.firstObjectToolButtonIndex]);this.createButton(d,UISpriteSheet.Eraser,this.changeTool.bind(this),"data-tool",EditorView.ToolDeleteObject.toString());this.buttonsWithLargeMargin.push(this.createButton(d,UISpriteSheet.Open,this.open.bind(this)));this.createButton(d,UISpriteSheet.Download,this.download.bind(this));this.createButton(d,UISpriteSheet.Clear,this.clear.bind(this));this.createButton(d,UISpriteSheet.Accept,
this.accept.bind(this)).style.float="right";this.createButton(d,UISpriteSheet.Play,this.play.bind(this)).style.float="right";this.fileInput.onchange=this.openFile.bind(this);this.selectionView=!!b;this.boundDocumentKeyUp=androidWrapper||isPWA?null:this.documentKeyUp.bind(this);this.tool=EditorView.ToolPencil;this.dirty=!1;this.context=null;this.contextGlobalCompositeOperation="source-over";this.lastBrushWidth=this.lastPencilRainbowIndex=this.lastPencilColor=0;this.lastObjectType=LevelObject.TypeBall;
this.brushContext=this.brushCanvas=null;this.loadOptions=a||null;this.level=null;this.levelBallCount=0;this.pointerHandler=null;this.pointerLastY=this.pointerLastX=0;this.pointerCursorAttached=!1;this.objectImages=[];this.objectImageDrag=null;this.keyUpCounter=this.keyUpTime=this.firstObjectToolButtonIndex=this.firstLineWidthButtonIndex=this.firstPencilButtonIndex=this.objectImageDragOffsetYCss=this.objectImageDragOffsetXCss=0;this.highlightTool()}setGridBackground(){this.scrollContainer.containerElement.style.backgroundImage=
...
class GameView extends View{constructor(a,b){super();this.baseElement.style.cursor="crosshair";this.baseElement.style.touchAction="none";const c=this.createButton(b?this.baseElement:null,UISpriteSheet.Back,this.back.bind(this));c.style.position="absolute";c.style.left="0";c.style.top="0";this.backButton=c;(this.preview=b)?this.timeDisplay=this.editNameButton=this.timeDisplayImage=this.restartButton=null:(b=this.createButton(null,UISpriteSheet.Restart,this.restart.bind(this)),b.style.position="absolute",
b.style.top="0",this.restartButton=b,b=document.createElement("div"),b.style.position="absolute",this.timeDisplayImage=UISpriteSheet.create(UISpriteSheet.Clock,b),b.appendChild(this.timeDisplayText=document.createTextNode(Strings.Time)),this.editNameButton=this.createButton(b,UISpriteSheet.Edit,this.editName.bind(this)),this.timeDisplay=b);b=this.createButton(this.baseElement,UISpriteSheet.Pause,this.pause.bind(this));b.style.position="absolute";b.style.right="0";b.style.top="0";this.resourceStorage=
new ResourceStorage;this.boundRender=this.render.bind(this);this.alive=!0;this.alreadyCreated=this.finished=this.paused=!1;this.loadOptions=a;this.viewY=this.level=null;this.frameRequest=0;this.finalUIAttached=!1;this.levelTexture=this.pointerCursorY=this.pointerCursorX=this.pointerCursorCenterY=this.pointerCursorCenterX=this.victory=this.totalElapsedMilliseconds=this.pointerCursorAttached=this.pointerHandler=null}get usesGL(){return!0}releaseResources(){this.alive&&this.resourceStorage.release()}loadResources(){this.alive&&
...
this.viewY=null;a?this.resourceStorage.release():this.resourceStorage.destroy()}restart(){if(!this.alive)return!1;this.finalUIAttached&&(this.finalUIAttached=!1,this.preview||(this.baseElement.removeChild(this.backButton),this.baseElement.removeChild(this.restartButton),this.baseElement.removeChild(this.timeDisplay)));this.finished=this.paused=!1;this.level.restart();const a=cLib.HEAP8.buffer;let b=cLib._getFirstPropertyPtr(this.level.levelPtr);this.viewY=new Float32Array(a,cLib._getViewYPtr(this.level.levelPtr),
...
{let f=d.name;/.*\(\d+\)$/.test(f)&&(f=f.substr(0,f.lastIndexOf("(")).trim(),f||(f=Strings.Level));let g=0,h=d.name;for(;;){let l=!0;for(let m=e.length-1;0<=m;m--)if(e[m]===h){l=!1;break}if(l)break;g++;h=`${f} (${g})`}d.name=h;LevelCache.saveLevel(d,!1).then(()=>{View.loading=!1;this.createThumbnail(h,!1);this.resize();Modal.show({title:Strings.Success,html:Strings.LevelImported+UISpriteSheet.html(UISpriteSheet.Success)})},c)},c)},c)):c()}else View.loading=!1,Modal.show({html:Strings.EmptyLevel+UISpriteSheet.html(UISpriteSheet.Error)})};
...
h)|0;0>d&&(d=0);baseTopCss=d/pixelRatio;main.style.left=baseLeftCss+"px";main.style.top=baseTopCss+"px";b>a||baseTopCss?landscapeWarning||(landscapeWarning=document.createElement("div"),landscapeWarning.style.pointerEvents="none",landscapeWarning.style.fontSize="2em",landscapeWarning.style.textAlign="center",landscapeWarning.style.position="absolute",landscapeWarning.style.left="0",landscapeWarning.style.top="0.5em",landscapeWarning.style.width="100%",landscapeWarning.style.zIndex="9999",landscapeWarning.textContent=
