...
(LevelCache.isNameValid(d.name)||(d.name=Strings.Level),d.prepare().then(()=>{LevelCache.getLevelNames().then(e=>{let f=d.name;/.*\(\d+\)$/.test(f)&&(f=f.substr(0,f.lastIndexOf("(")).trim(),f||(f=Strings.Level));let g=0,h=d.name;for(;;){let k=!0;for(let m=e.length-1;0<=m;m--)if(e[m]===h){k=!1;break}if(k)break;g++;h=`${f} (${g})`}d.name=h;LevelCache.saveLevel(d,!1).then(()=>{View.loading=!1;this.createThumbnail(h,!1);this.resize();Modal.show({title:Strings.Success,html:Strings.LevelImported+UISpriteSheet.html(UISpriteSheet.Success)})},
...
fullscreenChanged);View.createInitialView();View.loading=!1;a=document.createElement("script");a.async=!0;a.setAttribute("type","text/javascript");a.setAttribute("charset","utf-8");a.setAttribute("src","assets/js/levels.js");document.body.appendChild(a)})}window.pixelStepMAIN=!0;window.pixelCheckSetup&&window.pixelCheckSetup();
