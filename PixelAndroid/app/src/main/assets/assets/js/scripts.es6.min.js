...
c.length-1;0<=d;d--)c[d]&4278190080&&(c[d]=a);this.brushContext.putImageData(b,0,0)}clearImage(){this.dirty=!0;this.context.clearRect(0,0,baseWidth,maxHeight);this.level.clearImage();this.scrollContainer.scrollTo(0)}findObjectWithinRadius(a,b,c){const d=this.level.objects,e=d.length,f=iconSize*iconSize;for(let h=0;h<e;h++){var g=d[h];if(0<=a&&g.type!==a)continue;let l=b-g.x;g=c-g.y;l*=l;g*=g;if(f>=l+g)return this.objectImages[h]}return null}addObject(a,b){if(!b){if(this.level.objects.length>=Level.MaxObjectCount)return Modal.show({html:Strings.TooManyObjects+
