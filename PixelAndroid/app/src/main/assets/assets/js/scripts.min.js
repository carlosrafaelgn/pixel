...
this.containerElement.appendChild(this.modalElement);main.appendChild(this.containerElement);this.boundDocumentKeyDown=this.documentKeyDown.bind(this);document.addEventListener("keydown",this.boundDocumentKeyDown,!0);this.fading=!0;setTimeout(()=>{if(a.onshowing)a.onshowing();this.resizeInternal();this.containerElement.classList.add("visible");setTimeout(()=>{this.fading=!1;View.pushHistoryStateIfNecessary();if(this.options.onshown)this.options.onshown()},520)},50)}static get visible(){return!!Modal.modal}static get currentModalElement(){return Modal.modal?
...
css(43+iconSize),View.divLoading.style.height=css(11+iconSize))}static windowResized(a){if(8>baseLeftCss){if(!fadeLeft.style.backgroundColor){const b=androidWrapper||isPWA?"#99f":"#000";fadeLeft.style.backgroundColor=b;fadeRight.style.backgroundColor=b;fadeLeft.style.backgroundImage="none";fadeRight.style.backgroundImage="none"}}else fadeLeft.style.backgroundColor&&(fadeLeft.style.backgroundColor="",fadeRight.style.backgroundColor="",fadeLeft.style.backgroundImage="",fadeRight.style.backgroundImage=
"");a&&(View.resizeLoading(),baseTopCss?fadeLeft.style.display||(fadeLeft.style.display="none",fadeRight.style.display="none"):fadeLeft.style.display&&(fadeLeft.style.display="",fadeRight.style.display=""),glCanvas.width=baseWidth*scaleFactor,glCanvas.height=baseHeight*scaleFactor,glCanvas.style.height=baseHeightCss+"px",View.gl.checkRecreate(glCanvas)&&View.recreateResources(),View.currentView&&View.currentView.resize())}static windowHistoryStatePopped(a){View.windowHistoryStatePushed=!1;View.pushHistoryStateIfNecessary();
...
cover.classList.remove("visible");setTimeout(()=>{View._fading=!1;View.popHistoryStateIfNecessary();document.body.removeChild(cover);this.fadeInFinished();a()},520)},50)}async fadeIn(){if(View._fading||View.currentView===this)return null;View._fading=!0;View.currentView=this;return new Promise((a,b)=>{if((b=this.initialElements)&&!this.attached){for(let c=0;c<b.length;c++){const d=b[c];d&&main.appendChild(d)}this.attached=!0;if(b=this.attach()){b.then(()=>{this.finishFadeIn(a)},c=>{console.error(c)});
return}}this.finishFadeIn(a)})}async fadeOut(a){if(View._fading||View.currentView!==this)return null;View._fading=!0;View.backgroundFrameRequest&&(cancelAnimationFrame(View.backgroundFrameRequest),View.backgroundFrameRequest=0);return new Promise((b,c)=>{document.body.appendChild(cover);setTimeout(()=>{cover.classList.add("visible");setTimeout(async()=>{await this.destroy(a);a&&View.viewStack.push(this);View.currentView=null;View._fading=!1;View.pushHistoryStateIfNecessary();b()},520)},50)})}async fadeTo(a,
b=!1){if(View._fading||View.currentView!==this)return null;await this.fadeOut(b);return("string"===typeof a?new viewClasses[a]:a).fadeIn()}async fadeToPrevious(){if(View._fading||View.currentView!==this||!View.viewStack.length)return null;await this.fadeOut(!1);return View.viewStack.pop().fadeIn()}}View._loading=!1;View._fading=!1;View.currentView=null;View.viewStack=[];View.divLoading=null;View.divLoadingTimeout=0;View.windowHistoryStatePushed=!1;View.gl=null;View.sheetTexture=null;
...
a+" "+a;this.scrollContainer.resize(toolbarTotalHeight,baseHeight-(toolbarTotalHeight<<1));this.pointerCursor.style.borderWidth=borderWidthCss;this.pointerCursor.style.width=c;this.pointerCursor.style.height=c;a=this.level.objects;for(b=a.length-1;0<=b;b--){c=a[b];const d=this.objectImages[b];d.style.left=css(c.x-iconRadius);d.style.top=css(c.y-iconRadius);UISpriteSheet.resize(d)}super.resize()}async attach(){glCanvas.style.display="none";if(!this.level){this.level=await LevelCache.loadLevelFromOptions(this.loadOptions);
...
this.boundDocumentKeyUp&&document.addEventListener("keyup",this.boundDocumentKeyUp,!0)}async detach(){glCanvas.style.display="";this.pointerHandler&&(this.pointerHandler.destroy(),this.pointerHandler=null);this.scrollContainer.detach();this.brushContext=this.brushCanvas=this.context=null;this.boundDocumentKeyUp&&document.removeEventListener("keyup",this.boundDocumentKeyUp,!0)}destroyInternal(a){this.save()}save(){if(this.dirty||!this.level.image)this.level.modifiedAt=(new Date).getTime(),this.level.createdAt||
...
iconSizeCss,this.editNameButton.style.marginTop="-"+buttonMarginCss,UISpriteSheet.resize(this.timeDisplayImage),this.timeDisplayImage.style.marginRight=buttonMarginCss);this.level.viewResized();super.resize()}async attach(){this.level=await LevelCache.loadLevelFromOptions(this.loadOptions);this.loadOptions=null;this.pointerHandler=new PointerHandler(glCanvas,this.mouseDown.bind(this),this.mouseMove.bind(this),this.mouseUp.bind(this));this.levelTexture||(this.levelTexture=new Texture(View.gl,await loadImage(this.level.processedImage)),
this.resourceStorage.add("levelTexture",this.levelTexture));if(androidWrapper)androidWrapper.setKeepScreenOn(!0);else try{const a=navigator.wakeLock;a&&a.request&&!GameView.wakeLockSentinel&&(GameView.wakeLockSentinel=await a.request("screen"))}catch(a){}}async detach(){this.pointerHandler&&(this.pointerHandler.destroy(),this.pointerHandler=null);if(androidWrapper)androidWrapper.setKeepScreenOn(!1);else if(GameView.wakeLockSentinel)try{GameView.wakeLockSentinel.release&&(await GameView.wakeLockSentinel.release(),
GameView.wakeLockSentinel=null)}catch(a){}}destroyInternal(a){this.alreadyCreated=this.alive=!1;this.level.destroyLevelPtr();this.pointerCursorY=this.pointerCursorX=this.pointerCursorCenterY=this.pointerCursorCenterX=this.victory=this.totalElapsedMilliseconds=this.pointerCursorAttached=this.viewY=null;a?this.resourceStorage.release():this.resourceStorage.destroy()}restart(){if(!this.alive)return!1;this.finalUIAttached&&(this.finalUIAttached=!1,this.preview||(main.removeChild(this.backButton),main.removeChild(this.restartButton),
main.removeChild(this.timeDisplay)));this.finished=this.paused=!1;this.level.restart();const a=cLib.HEAP8.buffer;let b=cLib._getFirstPropertyPtr(this.level.levelPtr);this.viewY=new Float32Array(a,cLib._getViewYPtr(this.level.levelPtr),1);this.pointerCursorAttached=new Int32Array(a,b,1);b+=4;this.totalElapsedMilliseconds=new Int32Array(a,b,1);b+=4;this.victory=new Int32Array(a,b,1);b+=4;this.pointerCursorCenterX=new Float32Array(a,b,1);b+=4;this.pointerCursorCenterY=new Float32Array(a,b,1);b+=4;this.pointerCursorX=
new Float32Array(a,b,1);this.pointerCursorY=new Float32Array(a,b+4,1);this.frameRequest&&(cancelAnimationFrame(this.frameRequest),this.frameRequest=0);this.frameRequest=requestAnimationFrame(this.boundRender);return!0}back(a){if(!this.alive)return!1;this.alive=!1;this.paused=!0;this.frameRequest&&(cancelAnimationFrame(this.frameRequest),this.frameRequest=0);this.fadeToPrevious();return!0}pause(a){if(!this.alive||!this.paused&&(a=ControlMode.accelerationSupported?UISpriteSheet.html(ControlMode.modeImage):
null,!Modal.show({title:Strings.Pause,html:(androidWrapper||isPWA?"":`<button type="button" id="fullscreen" data-style="margin-bottom: ${buttonMargin}">${UISpriteSheet.html(UISpriteSheet.Fullscreen)}${Strings.Fullscreen}</button><br/>`)+(a?`<button type="button" id="controlMode" data-style="margin-bottom: ${buttonMargin}">${a}${Strings.ControlMode}</button><br/>`:"")+`<button type="button" id="restart">${UISpriteSheet.html(UISpriteSheet.Restart)}${Strings.Restart}</button>`,buttons:[{iconId:UISpriteSheet.Back,
text:Strings.Exit,onclick:()=>{Modal.hide();this.back(null)}},{defaultCancel:!0,iconId:UISpriteSheet.PlayGreen,text:Strings.Play,className:"accept",onclick:()=>{Modal.hide();this.pause(null)}}],onbuttonclick:b=>{switch(b){case "fullscreen":fullscreenControl.toggleFullscreen();break;case "controlMode":ControlMode.toggleMode();UISpriteSheet.change(document.getElementById("controlMode").firstChild,ControlMode.modeImage);break;case "restart":this.restart(),Modal.hide()}}})))return!1;this.paused=!this.paused;
...
buttonMarginCss;super.resize()}async attach(){glCanvas.style.cursor="default"}async detach(){glCanvas.style.cursor=""}destroyInternal(a){}play(a){this.fadeTo("SelectionView");return!0}edit(a){this.fadeTo("EditorView");return!0}fullscreen(a){fullscreenControl.toggleFullscreen();return!0}exit(a){androidWrapper.exit();return!0}about(a){a=[{defaultCancel:!0,iconId:UISpriteSheet.Back,text:Strings.Close,onclick:Modal.hide}];!androidWrapper&&installationPrompt&&installationPrompt.prompt&&a.push({iconId:UISpriteSheet.Download,
text:Strings.Install,onclick:(b,c)=>{if(installationPrompt)try{b=installationPrompt,installationPrompt=null,b.prompt()}catch(d){}setTimeout(()=>{c.style.display="none"},blinkTotalDurationMS)}});Modal.show({title:Strings.About+` <span style="float: right;">v${version}${window.pixelES2017?"":"es6"}</span>`,large:!0,html:Strings.About1+(2===View.gl.contextVersion?Strings.About2:"")+Strings.About3+(window.pixelUsingWebAssembly?"":Strings.About4)+Strings.About5+UISpriteSheet.html(UISpriteSheet.Success)+
...
h)|0;0>d&&(d=0);baseTopCss=d/pixelRatio;main.style.left=baseLeftCss+"px";main.style.top=baseTopCss+"px";d=`scale(${Math.ceil(.25*a)},${Math.ceil(.25*b)})`;cover.style.webkitTransform=d;cover.style.mozTransform=d;cover.style.transform=d;b>a||baseTopCss?landscapeWarning||(landscapeWarning=document.createElement("div"),landscapeWarning.style.pointerEvents="none",landscapeWarning.style.fontSize="2em",landscapeWarning.style.textAlign="center",landscapeWarning.style.position="absolute",landscapeWarning.style.left=
"0",landscapeWarning.style.top="0.5em",landscapeWarning.style.width="100%",landscapeWarning.style.zIndex="9999",landscapeWarning.textContent=Strings.LandscapeWarning,document.body.appendChild(landscapeWarning)):landscapeWarning&&(document.body.removeChild(landscapeWarning),landscapeWarning=null);scaleFactor!==c||baseHeight!==f?(main.style.width=baseWidthCss+"px",main.style.height=baseHeightCss+"px",maxHeightCss=cssNumber(maxHeight),thumbnailWidthCss=css(thumbnailWidth),thumbnailHeightCss=css(thumbnailHeight),
buttonHeightCss=css(buttonHeight),borderWidthCss=css(borderWidth),toolbarAvailableHeightCss=css(toolbarAvailableHeight),toolbarTotalHeightCss=css(toolbarTotalHeight),iconSizeCss=css(iconSize),buttonMarginCss=css(buttonMargin),buttonLargeMarginCss=css(buttonLargeMargin),buttonPaddingCss=css(buttonPadding),fontSizeCss=css(8),document.body.style.fontSize=fontSizeCss,LevelSpriteSheet.recreateIfNecessary(),UISpriteSheet.windowResized(),View.windowResized(!0),Modal.windowResized()):View.windowResized(!1)}
