...
""}setTimeDisplayTextRecord(a){this.timeDisplayText.nodeValue=LevelCache.formatLevelRecordTime(this.totalElapsedMilliseconds[0])+" - "+(a||Strings.NoName)}editName(){this.alive&&this.level&&this.totalElapsedMilliseconds&&this.victory&&this.victory[0]&&Modal.show({title:Strings.NewRecord,html:`<label for="name">${Strings.Name}</label><input id="name" spellcheck="false" autocomplete="off" maxlength="8" /><br/><label>${Strings.Time}</label><label>${LevelCache.formatLevelRecordTime(this.totalElapsedMilliseconds[0])}</label>`,
okcancel:!0,okcancelsubmit:!0,onshowing:()=>{const a=Modal.currentModalHeaderElement;if(a){a.setAttribute("data-style","line-height:"+iconSize);const b=UISpriteSheet.create(UISpriteSheet.Trophy);b.setAttribute("data-style","margin-right:"+buttonMargin);a.insertBefore(b,a.firstChild);UISpriteSheet.create(UISpriteSheet.Trophy,a).setAttribute("data-style","margin-left:"+buttonMargin)}document.getElementById("name").value=LevelCache.LastRecordName||""},onshown:()=>{document.getElementById("name").focus()},
onok:()=>__awaiter(this,void 0,void 0,function*(){if(this.alive&&this.level&&this.totalElapsedMilliseconds){var a=document.getElementById("name").value.trim();a?LevelCache.setLevelRecord(this.level.name,this.totalElapsedMilliseconds[0],a):(LevelCache.deleteLevelRecord(this.level.name),LevelCache.clearLastRecordName());this.setTimeDisplayTextRecord(a);Modal.hide()}})})}render(a){if(this.alive){this.frameRequest=requestAnimationFrame(this.boundRender);var b=this.level;View.drawBackground(a,b.levelPtr,
